...
// definition of an additional function added by Zach Monte on 03/23/2021
void extern init_fix() {
	_sys_timer = TimerCore(get_slot_addr(BRIDGE_BASE, TIMER_SLOT));
	uart = UartCore(get_slot_addr(BRIDGE_BASE, UART_SLOT));
}
...
// current system time in clock cycles
uint64_t read_cycle()
{
   return _sys_timer.read_tick();
}
